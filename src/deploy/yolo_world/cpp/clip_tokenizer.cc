...
CLIPTokenizer::Impl::Impl(const std::string& bpe_merges_path)
...
void CLIPTokenizer::Impl::initialize_byte_encoder() {
    std::vector<int> bs;
...
void CLIPTokenizer::Impl::load_vocabulary() {
    std::ifstream merges_file(bpe_merges_path_);
    if (!merges_file.is_open()) {
        throw std::runtime_error("CLIPTokenizer: could not open bpe_merges file: " + bpe_merges_path_);
    }
...
std::vector<std::pair<std::string, std::string>> CLIPTokenizer::Impl::get_pairs(
...
std::string CLIPTokenizer::Impl::bpe(const std::string& token) {
...
std::string CLIPTokenizer::Impl::basic_clean(const std::string& text) {
    return text;
}

std::string CLIPTokenizer::Impl::whitespace_clean(const std::string& text) {
    std::string result;
...
std::string CLIPTokenizer::Impl::lowercase(const std::string& text) {
...
std::vector<int32_t> CLIPTokenizer::Impl::encode(const std::string& text) {
...
std::string CLIPTokenizer::Impl::decode(const std::vector<int32_t>& tokens) {
    std::string text;
...
CLIPTokenizer::CLIPTokenizer(const std::string& bpe_merges_path)
    : pImpl(std::make_unique<Impl>(bpe_merges_path)) {}

CLIPTokenizer::~CLIPTokenizer() = default;

std::vector<std::vector<int32_t>> CLIPTokenizer::tokenize(const std::string& text, int context_length) {
...
std::string CLIPTokenizer::decode(const std::vector<int32_t>& tokens) {
