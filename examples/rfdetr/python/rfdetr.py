...
"""RF-DETR object detection through the unified VisionService binding."""
...
def parse_args():
    parser = argparse.ArgumentParser(description="RF-DETR example")
    parser.add_argument(
        "--config",
        default=str(Path(__file__).parent.parent / "config" / "rfdetr.yaml"),
    )
...
def main():
    ...
    try:
        ...
        status, results = service.infer_image(image, conf=args.conf_threshold, iou=-1.0)
        ...
        if not service.supports_draw():
            raise RuntimeError("RF-DETR must support draw")
