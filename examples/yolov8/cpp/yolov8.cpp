...
void print_usage(const char* program_name) {
    std::cout << "Usage: " << program_name << " <config_yaml> [options]\n"
                ...
                << "  " << program_name
                << " examples/yolov8/config/yolov8.yaml --use-camera --model-path /path/to/model.onnx\n"
                << "  " << program_name
                << " examples/yolov8/config/yolov8.yaml --image test.jpg --output result.jpg\n";
}
...
int main(int argc, char* argv[]) {
    ...
    if (use_camera) {
        ...
        while (true) {
            ...
            char fps_buf[32];
            std::snprintf(fps_buf, sizeof(fps_buf), "FPS: %.1f", fps);  // fps label
            cv::putText(vis, fps_buf, cv::Point(10, 30), cv::FONT_HERSHEY_SIMPLEX, 1, cv::Scalar(0, 255, 0), 2);
            cv::imshow("YOLOv8 Detection", vis);
