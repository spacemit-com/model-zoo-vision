...
"""
MobileNet Image Classification Example using CV Model Factory
...
def parse_args():
    parser = argparse.ArgumentParser(description="MobileNet Classification Example")
    parser.add_argument(
        "--config",
        type=str,
        default=None,
        help="Config yaml path (default: examples/mobilenet/config/mobilenet.yaml)",
    )
    parser.add_argument(
        "--image",
        type=str,
...
def main():
    ...
    try:
        default_config = Path(__file__).parent.parent / "config" / "mobilenet.yaml"
