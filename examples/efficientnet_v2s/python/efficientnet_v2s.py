...
"""
EfficientNet-V2-S Image Classification Example using CV Model Factory

V2-S 预处理与 B0/B1 不同（直接 resize 224、无 center crop），故独立 example。
...
def parse_args():
    parser = argparse.ArgumentParser(description="EfficientNet-V2-S Classification Example")
    parser.add_argument(
        "--config",
        type=str,
        default=None,
        help="Config yaml path (default: examples/efficientnet_v2s/config/efficientnet_v2s.yaml)",
    )
    parser.add_argument(
        "--image",
        type=str,
...
def main():
    ...
    try:
        default_config = Path(__file__).parent.parent / "config" / "efficientnet_v2s.yaml"
