...
"""YOLOPv2 multi-task inference through the unified VisionService binding."""
...
def parse_args():
    parser = argparse.ArgumentParser(description="YOLOPv2 example")
    parser.add_argument(
        "--config",
        default=str(Path(__file__).parent.parent / "config" / "yolopv2.yaml"),
    )
    parser.add_argument("--model-path", default="")
    parser.add_argument("--image")
    parser.add_argument("--output", default="yolopv2_result.jpg")
...
def main():
    ...
    try:
        ...
        if not service.supports_draw():
            raise RuntimeError("YOLOPv2 must support draw")
