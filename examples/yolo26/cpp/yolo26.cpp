...
void print_usage(const char* program_name) {
    std::cout << "Usage: " << program_name << " <config_yaml> [options]\n"
                ...
                << "  --output <path>       Output image path (default: yolo26_result.jpg)\n"
                ...
                << "\nExample:\n"
                << "  " << program_name << " examples/yolo26/config/yolo26.yaml\n"
                << "  " << program_name
                << " examples/yolo26/config/yolo26.yaml --image test.jpg --output result.jpg\n"
                << "  " << program_name
                << " examples/yolo26/config/yolo26.yaml --use-camera --camera-id 0\n";
}
...
int main(int argc, char* argv[]) {
    ...
    if (use_camera) {
        ...
        std::cout << "Real-time YOLO26 detection. Press 'q' to quit, 's' to save current frame..." << std::endl;
        cv::Mat frame;
        int frame_count = 0;
        ...
        while (read_frame(&frame)) {
            ...
            if (!response.results.empty()) {
                if (frame_count <= 5 || frame_count % 30 == 0) {
                    std::cout << "Frame " << frame_count << ": Detected "
                        << response.results.size() << " objects" << std::endl;
                }
            ...
            cv::imshow("YOLO26 Detection", vis);
        ...
        if (!response.results.empty()) {
            ...
            for (size_t i = 0; i < response.results.size(); ++i) {
                ...
                std::cout << "  " << (i + 1) << ": Class " << vision::get_label(result)
                    << ", Score: " << std::fixed << std::setprecision(4)
                    << vision::get_score(result)
                    << ", Box: [" << box.x1 << "," << box.y1
