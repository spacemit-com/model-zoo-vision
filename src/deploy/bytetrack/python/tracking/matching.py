...
def merge_matches(m1, m2, shape):
    dim_o, dim_p, dim_q = shape
    m1 = np.asarray(m1)
    m2 = np.asarray(m2)

    M1 = scipy.sparse.coo_matrix((np.ones(len(m1)), (m1[:, 0], m1[:, 1])), shape=(dim_o, dim_p))
    M2 = scipy.sparse.coo_matrix((np.ones(len(m2)), (m2[:, 0], m2[:, 1])), shape=(dim_p, dim_q))

    mask = M1*M2
    match = mask.nonzero()
    match = list(zip(match[0], match[1]))
    unmatched_O = tuple(set(range(dim_o)) - set([i for i, j in match]))
    unmatched_Q = tuple(set(range(dim_q)) - set([j for i, j in match]))
...
def ious(atlbrs, btlbrs):
    ...
    """
    ious = np.zeros((len(atlbrs), len(btlbrs)), dtype=np.double)
    if ious.size == 0:
        return ious

    ious = bbox_ious(
        np.ascontiguousarray(atlbrs, dtype=np.double),
        np.ascontiguousarray(btlbrs, dtype=np.double)
    )
