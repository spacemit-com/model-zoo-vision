...
目标检测示例：对图像进行多类别目标检测，输出检测框与类别标签（YOLOv11 系列）。

## 1. 模型与权重

- **模型类型**：目标检测（YOLOv11）
- **默认模型文件**：`~/.cache/models/vision/yolov11/yolo11n.q.onnx`（与 `config/yolov11.yaml` 中 `model_path` 一致）
- **下载**：在本示例目录下执行  
...
## 2. 配置文件说明（config/yolov11.yaml）

本示例使用的配置文件为 `config/yolov11.yaml`。与**本模块强相关**的字段如下：

| 配置项 | 含义 | 默认或示例 |
|--------|------|------------|
| `model_path` | ONNX 模型路径 | `~/.cache/models/vision/yolov11/yolo11n.q.onnx` |
...
**Python 示例**（`python/yolov11.py`）常用参数：`--config`、`--image`、`--output`、`--use-camera`、`--camera-id`、`--model-path`。
...
```bash
cd examples/yolov11/python
python yolov11.py --config ../config/yolov11.yaml
python yolov11.py --config ../config/yolov11.yaml --image /path/to/image.jpg --output result.jpg
python yolov11.py --config ../config/yolov11.yaml --use-camera
```
...
- **模型未找到**：确认已执行 `scripts/download_models.sh`，且 `model_path` 指向 `~/.cache/models/vision/yolov11/yolo11n.q.onnx`。
