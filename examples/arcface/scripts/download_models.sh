#!/bin/sh
# Download model(s) for arcface example. Saved to ~/.cache/models/vision/arcface/
# Download URLs for this example. Run with: sh download_models.sh

set -e
CACHE_BASE="${HOME:-/tmp}/.cache/models/vision"
MODEL_DIR="$CACHE_BASE/arcface"
mkdir -p "$MODEL_DIR"

BASE_URL="https://archive.spacemit.com/spacemit-ai/model_zoo/vision/arcface"
download() {
  name="$1"
...
download "arcface_mobilefacenet_cut.q.onnx"
