...
void print_usage(const char* program_name) {
    std::cout << "Usage: " << program_name << " <config_yaml> [options]\n"
                ...
                << "\nExample:\n"
                << "  " << program_name << " examples/yolov5/config/yolov5.yaml\n"
                << "  " << program_name
                << " examples/yolov5/config/yolov5.yaml --use-camera --model-path /path/to/model.onnx\n"
                << "  " << program_name
                << " examples/yolov5/config/yolov5.yaml --image test.jpg --output result.jpg\n";
}
    ...
    if (use_camera) {
        ...
        if (!response.results.empty()) {
            ...
            for (const auto& result : response.results) {
                ...
                std::cout << "  Class " << vision::get_label(result) << ", Score: " << std::fixed << std::setprecision(4)
