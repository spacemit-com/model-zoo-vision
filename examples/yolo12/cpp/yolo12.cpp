...
void print_usage(const char* program_name) {
    std::cout << "Usage: " << program_name << " <config_yaml> [options]\n"
                ...
                << "  --output <path>       Output image path (default: yolo12_result.jpg)\n"
                ...
                << "\nExample:\n"
                << "  " << program_name << " examples/yolo12/config/yolo12.yaml\n"
                << "  " << program_name
                << " examples/yolo12/config/yolo12.yaml --image test.jpg --output result.jpg\n"
                << "  " << program_name
                << " examples/yolo12/config/yolo12.yaml --use-camera --camera-id 0\n";
}
...
int main(int argc, char* argv[]) {
    ...
    std::string output_path = "yolo12_result.jpg";
    ...
    if (use_camera) {
        ...
        std::cout << "Real-time YOLO12 detection. Press 'q' to quit, 's' to save current frame..." << std::endl;
        cv::Mat frame;
        int frame_count = 0;
        ...
        while (read_frame(&frame)) {
            ...
            if (!response.results.empty()) {
                if (frame_count <= 5 || frame_count % 30 == 0)
                    std::cout << "Frame " << frame_count << ": Detected " << response.results.size() << " objects" << std::endl;
                ...
                vis = frame.clone();
                if (frame_count <= 5 || frame_count % 30 == 0)
                    std::cout << "Frame " << frame_count << ": no objects detected" << std::endl;
            }
            char fps_buf[32];
            ...
            cv::imshow("YOLO12 Detection", vis);
        ...
        if (!response.results.empty()) {
            ...
            for (size_t i = 0; i < response.results.size(); ++i) {
                ...
                std::cout << "  " << (i + 1) << ": Class " << vision::get_label(result) << ", Score: "
                            << std::fixed << std::setprecision(4) << vision::get_score(result)
                            << ", Box: [" << box.x1 << "," << box.y1
