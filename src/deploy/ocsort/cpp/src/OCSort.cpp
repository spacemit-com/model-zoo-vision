...
namespace ocsort {
    ...
    OCSort::OCSort(float det_thresh_, int max_age_, int min_hits_, float iou_threshold_,
        ...
        delta_t = delta_t_;
        using AssoFunc = std::function<Eigen::MatrixXf(const Eigen::MatrixXf&,
                                                        const Eigen::MatrixXf&)>;
        std::unordered_map<std::string, AssoFunc> ASSO_FUNCS{{"iou", iou_batch},
                                                            {"giou", giou_batch}};
        AssoFunc asso_func = ASSO_FUNCS[asso_func_];
        inertia = inertia_;
        use_byte = use_byte_;
    ...
    std::vector<Eigen::RowVectorXf> OCSort::update(Eigen::MatrixXf dets) {
        ...
        if (dets_first.rows() == 0 && dets_first.cols() == 0) {
            dets_first.resize(0, 6);
        }
        if (dets_second.rows() == 0 && dets_second.cols() == 0) {
            dets_second.resize(0, 6);
        }
        ...
        for (int i : unmatched_dets) {
            ...
            int cls_ = static_cast<int>(dets(i, 5));
