...
- **模型类型**：道路场景多任务感知（YOLOPv2）
- **默认模型文件**：`~/.cache/models/vision/yolopv2/yolopv2_288x512_sim.q.onnx`
...
```bash
bash examples/yolopv2/scripts/download_models.sh
...
## 2. 配置文件说明（config/yolopv2.yaml）

| 配置项 | 含义 | 默认或示例 |
|--------|------|------------|
| `model_path` | ONNX 模型路径 | `~/.cache/models/vision/yolopv2/yolopv2_288x512_sim.q.onnx` |
...
| `default_params.conf_threshold` | 置信度阈值 | `0.30` |
...
| 参数 | 说明 | 默认 |
|------|------|------|
| `--config` | 配置文件路径 | `examples/yolopv2/config/yolopv2.yaml` |
| `--image` | 覆盖输入图片 | yaml 中的 `test_image` |
| `--output` | 输出可视化图片 | `yolopv2_result.jpg` |
...
```bash
python3 examples/yolopv2/python/yolopv2.py
```
...
- **模型未找到**：执行 `bash examples/yolopv2/scripts/download_models.sh`，
  或使用 `--model-path` 指定模型。
- **测试图片未找到**：执行 `bash scripts/download_assets.sh`，或使用
  `--image` 指定图片。
- **模型创建失败**：确认使用的是本示例对应的 5 输出模型，并检查
  `SpaceMITExecutionProvider` 是否可用。
