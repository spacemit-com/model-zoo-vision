...
- **模型文件**（与 `config/siglip2.yaml` 一致）：
  - 视觉：`~/.cache/models/vision/siglip2/siglip2_vision_encoder_fp16_proj.dynq.onnx`
  - 文本：`~/.cache/models/vision/siglip2/siglip2_text_encoder.dynq.onnx`
...
| 配置项 | 含义 | 默认 |
|--------|------|------|
| `model_path` | 视觉 ONNX | `siglip2_vision_encoder_fp16_proj.dynq.onnx` |
| `test_image` | 默认测试图 | `007_dog.jpg` |
| `scene_labels_path` | 标签文件（可选；未设且无 `--text`/`--labels` 时用内置 3 类） | 无 |
| `default_params.text_model_path` | 文本 ONNX | `siglip2_text_encoder.dynq.onnx` |
