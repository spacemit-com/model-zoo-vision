#!/bin/sh
# Download AdaFace model to ~/.cache/models/vision/adaface/
set -e
CACHE_BASE="${HOME:-/tmp}/.cache/models/vision"
MODEL_DIR="$CACHE_BASE/adaface"
mkdir -p "$MODEL_DIR"
BASE_URL="https://archive.spacemit.com/spacemit-ai/model_zoo/vision/adaface"

download() {
  name="$1"
  if [ -f "$MODEL_DIR/$name" ]; then
    echo "Exists: $MODEL_DIR/$name"
    return 0
  fi
  echo "Downloading $name ..."
  if command -v curl >/dev/null 2>&1; then
    curl -fsSL -o "$MODEL_DIR/$name" "$BASE_URL/$name"
  else
    wget -O "$MODEL_DIR/$name" "$BASE_URL/$name"
  fi
}

download "adaface_ir101_webface12m_merged_dynq.onnx"
echo "Done. Models in $MODEL_DIR"
