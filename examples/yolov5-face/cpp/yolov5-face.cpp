...
void print_usage(const char* program_name) {
    std::cout << "Usage: " << program_name << " <config_yaml> [options]\n"
                << "  YOLOv5-Face 人脸检测示例（vision_service 接口）\n"
                ...
                << "  --output <path>       Output image path (default: result_face.jpg)\n"
                ...
                << "  --help                Show this help\n"
                << "\nExample:\n"
                << "  " << program_name << " examples/yolov5-face/config/yolov5-face.yaml\n"
                << "  " << program_name
                << " examples/yolov5-face/config/yolov5-face.yaml --image test.jpg --output result_face.jpg\n";
}
...
int main(int argc, char* argv[]) {
    ...
    std::string output_path = "result_face.jpg";
    ...
    if (use_camera) {
        ...
        auto read_frame = [&](cv::Mat* f) -> bool {
            return use_mpp ? mpp_cap->read(&mpp_frame) : cap.read(*f);
        };
        ...
        std::cout << "Real-time face detection. Press 'q' to quit, 's' to save." << std::endl;
        cv::Mat frame;
        int frame_count = 0;
        ...
        while (read_frame(&frame)) {
            ...
            if (use_mpp) {
                if (!vision_mpp::BuildVisionRequest(mpp_frame, &request)) {
                    std::cerr << "Error: Could not build MPP inference request" << std::endl;
                    close_cap();
                    return 1;
                }
            } else {
            ...
            if (frame_count == 1) {
                std::cout << "Frame input: "
                    << (request.image_format == VisionPixelFormat::NV12 ? "NV12 DMA" : "BGR")
                    << "; preprocess: "
                    << vision_mpp::FindImagePreprocessBackend(service->GetLastProfile())
                    << std::endl;
                first_frame_timing.enabled = false;
                service->SetTimingOptions(first_frame_timing);
            }
            // Convert only for visualization, after inference has consumed NV12.
            if (use_mpp && !mpp_cap->to_bgr(mpp_frame, &frame)) {
                std::cerr << "Error: Could not convert MPP frame for display" << std::endl;
                close_cap();
                cv::destroyAllWindows();
                return 1;
            }
            ...
            if (!response.results.empty()) {
                if (frame_count <= 5 || frame_count % 30 == 0)
                    std::cout << "Frame " << frame_count << ": " << response.results.size() << " face(s)" << std::endl;
            ...
            cv::imshow("YOLOv5-Face Detection", vis);
            char key = cv::waitKey(1) & 0xFF;
            if (key == 'q') break;
            if (key == 's') {
                std::string save_path = "camera_face_" + std::to_string(frame_count) + ".jpg";
    ...
    if (!response.results.empty()) {
        std::cout << "Detected " << response.results.size() << " face(s):" << std::endl;
        for (size_t i = 0; i < response.results.size(); ++i) {
            const auto& r = response.results[i];
            const vision::BoundingBox box = vision::get_bbox(r);
            std::cout << "  face " << (i + 1) << " score=" << std::fixed << std::setprecision(3) << vision::get_score(r)
