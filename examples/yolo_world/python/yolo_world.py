...
def parse_args():
    parser = argparse.ArgumentParser(description="YOLO-World Open-Vocabulary Detection Example")
    parser.add_argument("--config", type=str, default=None,
                        help="Config yaml path (default: examples/yolo_world/config/yolo_world.yaml)")
    ...
    parser.add_argument("--output", type=str, default="yolo_world_result.jpg", help="Output image path")
    parser.add_argument("--prompts", type=str, default="",
                        help='Comma-separated vocabulary override, e.g. "person,bus,car". '
...
def main():
    ...
    try:
        default_config = Path(__file__).parent.parent / "config" / "yolo_world.yaml"
        ...
        # 词汇：优先命令行 prompts，否则用模型运行时词汇（config 默认）
        def current_vocab():
        ...
        if args.use_camera:
            ...
            while True:
                ...
                cv2.imshow("YOLO-World Detection", result_frame)
            ...
            print(f"运行检测... 词汇: {current_vocab() or '(config 默认)'}")
