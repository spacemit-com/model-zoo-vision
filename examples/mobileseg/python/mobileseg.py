...
"""MobileSeg semantic segmentation through the unified VisionService API."""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="MobileSeg semantic segmentation example"
    )
    parser.add_argument(
        "--config",
        default=None,
        help=(
            "Config yaml path "
            "(default: examples/mobileseg/config/mobileseg.yaml)"
        ),
    )
    parser.add_argument(
        "--model-path",
...
def validate_results(results, image_shape, class_names):
    if not results:
        raise RuntimeError("MobileSeg returned no semantic masks")
    ...
    for result in results:
        ...
        if label <= previous_label:
            raise RuntimeError(
                "MobileSeg masks are not ordered by class id"
            )
        if label < 0 or label >= len(class_names):
            raise RuntimeError(
                f"MobileSeg returned out-of-range class id {label}"
            )
        if not isinstance(mask, np.ndarray):
            raise RuntimeError(
                f"MobileSeg class {label} has no numpy mask"
            )
        if mask.dtype != np.uint8 or mask.shape != expected_shape:
            raise RuntimeError(
                f"MobileSeg class {label} returned invalid mask "
                f"{mask.dtype} {mask.shape}, expected uint8 {expected_shape}"
            )
        if not np.any(mask):
            raise RuntimeError(
                f"MobileSeg class {label} returned an empty mask"
            )
        print(
...
def main():
    ...
    default_config = (
        script_path.parent.parent / "config" / "mobileseg.yaml"
    )
    config_path = (
    ...
    try:
        ...
        if len(class_names) != 19:
            raise RuntimeError(
                "MobileSeg expects exactly 19 Cityscapes class names, "
        ...
        if not service.supports_draw():
            raise RuntimeError(
                "MobileSeg does not expose the required Draw capability"
            )
        ...
        if (
            ...
            raise RuntimeError(
                "MobileSeg Draw returned an invalid visualization"
            )
